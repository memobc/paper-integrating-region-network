...
model_summaries %>%
  filter(str_detect(Title, 'Features')) %>%
  createModelCompTbl(Model1Title = "Two Factor Model -- All Memory Features") %>% 
  mutate(Title = str_remove(Title, 'All ')) %>%
  select(-`Wald's Test`) -> modelFitInformation

createParamTbl(M$data.all_modelType.Structural_measureModel.TwoFactor_modelName.Features.out) %>% 
filter(BetweenWithin == 'Within') %>% 
select(paramHeader, param, est.stand, se.stand, pval.stand) %>%
mutate(paramHeader = remove_dup(paramHeader))  -> BaseModel
